<template>
  <div>
    <v-btn
      ...
      color="primary"
      dark
      v-on:click="searchFormDisplayed = !searchFormDisplayed"
    >
      Busqueda avanzada
    </v-btn>
    ...
    <v-form v-if="searchFormDisplayed" class="mb-2" v-model="valid">
      ...
      <v-btn
        color="primary"
        :disabled="!valid"
        v-on:click="search(animalSearched)"
        >Buscar</v-btn
      >
      <v-btn color="secondary" v-on:click="clear()">Borrar busqueda</v-btn>
      <v-btn color="secondary" v-on:click="searchFormDisplayed = false"
        >Close</v-btn
      >
    </v-form>
